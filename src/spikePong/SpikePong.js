...
function SpikePong() {
  ...
  useEffect(() => {
    ...
    rightPaddle.current = new Paddle(canvas.current.width - 10, canvas.current.height / 2 - 40, 10, 80);
    ...
    document.addEventListener("keydown", function(e) {
      ...
      switch(e.which) {
        ...
        case 37:
          //SPIKE THE BALL
          if (frameCount.current >= frameWhenRightSpikeActivated.current + 200 && !ball.current.isSpikingFromLeft) {
        ...
        case 83: //S key
          ...
          break;
        case 68:
          if (frameCount.current >= frameWhenLeftSpikeActivated.current + 200 && ball.current.isSpikingFromRight == false) {
  ...
  function animate() {
    ...
    if (playerTwoScore.current == 11) {
      context.current.fillText(playerTwoScore.current, canvas.current.width - 77, 60);
    ...
    else if (frameCount.current <= frameWhenRightSpikeActivated.current + 20) {
      rightPaddle.current.dx = 5;
    }
    else {
    ...
    if (playerTwoScore.current < 10) {
      context.current.fillText(playerTwoScore.current, canvas.current.width - 55, 60);
    }
    else {
      context.current.fillText(playerTwoScore.current, canvas.current.width - 77, 60);
    }

    if (ball.current.y + ball.current.radius >= canvas.current.height || ball.current.y - ball.current.radius <= 0) {
  ...
  function hitLeftPaddle() {
    ...
    if (ball.current.y <= leftPaddle.current.y + leftPaddle.current.height && ball.current.y >= leftPaddle.current.y) {
      //if the ball position is past the edge of the paddle AND it wasnt past the edge of the paddle in the previous frame
      if (ball.current.x - ball.current.radius <= leftPaddle.current.x + leftPaddle.current.width && 
        (ball.current.x - ball.current.radius + Math.abs(ball.current.dx) >= leftPaddle.current.x + leftPaddle.current.width - Math.abs(leftPaddle.current.dx))) {
        return true;
      }
    }
  ...
  function hitRightPaddle() {
    if (ball.current.y <= rightPaddle.current.y + rightPaddle.current.height && ball.current.y >= rightPaddle.current.y) {
      if (ball.current.x + ball.current.radius >= rightPaddle.current.x &&
      (ball.current.x + ball.current.radius - Math.abs(ball.current.dx) <= rightPaddle.current.x + rightPaddle.current.width + Math.abs(rightPaddle.current.dx))) {
        return true;
      }
    }
  ...
  function isPastEdgeOfScreen() {
    if (ball.current.x + ball.current.radius >= canvas.current.width + 3 * Math.abs(ball.current.dx) || ball.current.x - ball.current.radius <= -3 * Math.abs(ball.current.dx)) {
      return true;
    }
    return false;
  }
